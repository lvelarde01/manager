...
export async function loader( {params} ) {
    let result = await ActionFetch({_id:params.id},'/api/workers/getinfo');
    console.log(result)
    return result;
    }
...
export function Edit() {
    ...
    const result = await ActionFetch({...dataUserObj,_id:dataInfo._id},'/api/workers/edit');
    ...
    <div className={`row justify-content-center ${Auth.theme}-style`} >
        ...
        <fieldset>
        <div className="mb-3">
        <h2>MODIFICAR TRABAJOR</h2>
        </div>
        <legend>Informacion</legend>
        <div className="mb-3">
            <input type="text" name='name' defaultValue={dataInfo.name} className="form-control" required placeholder="Nombre y Apellido" onChange={handlerOnchange}/>
