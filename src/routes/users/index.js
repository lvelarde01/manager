...
export function Index() {
  ...
  useEffect(()=>{
    if(loadingData)return;
      ...
      const dataReceived = await ActionFetch({},'/api/users/list');
  ...
  const handlerSearch = async(value)=>{
    ...
    let dataFilter = dataRow.filter((data)=>{
      if(data.firstname.toLocaleLowerCase().includes(value) ||
      data.firstname.toLocaleLowerCase().includes(value)){
        return data
      }
    });
  ...
  const handlerDeleteAll = async(event)=>{
    ...
    const result = await ActionFetch(deleteAll,'/api/users/trashall');
  ...
  const handlerTrash = async(event)=>{
    ...
    let result = await ActionFetch({_id},'/api/users/trash');
    if(result.acknowledged){
      setTimeout(()=>{
        setFetchReady({ready:true,msgtype:'success',message:'Se Elimino correctamente.'});
      ...
      <table className="table table-hover">
          ...
          <tbody>
            {dataRowShow&&dataRowShow.map((data)=>(
                ...
                <td>{data.email}</td>
                <td>{data.role}</td>
                <td><Link className={`btn btn-primary ${loading?`disabled`:``}`} onClick={handlerEdit} to={`/users/edit/${data._id}`}><i className='fas fa-pencil'/></Link>
